...
def startInducedRadDam(datacollect_params, old={"xds_dir":None}):
    if not datacollect_params["xds_dir"] == old["xds_dir"]:
        ...
        eda_dirs=list(filter(os.path.isdir, [os.path.join(datacollect_params['EDNA_files_dir'], x) for x in os.listdir(datacollect_params['EDNA_files_dir']) if x.startswith("EDA")]))
        eda_dirs.sort()
