# Based on EMBLMachineInfo from Ivars Karpics (EMBL Hamburg)
"""
[Name] MachineInfo
...
class LNLSMachineInfo(Equipment):
    """
    ...
    def init(self):
        ...
        self.values_dict['intens1'] = self['intensity1'].getProperties()
        self.values_dict['intens1']['value'] = None
        self.values_dict['intens1']['ranges'] = []
        for intens_range in self['intensity1']['ranges']:
            temp_intens_range = {}
            ...
            self.values_dict['intens1']['ranges'].append(temp_intens_range)
        #Sort the ranges accordingly to the maximal values
        self.values_dict['intens1']['ranges'] = \
             sorted(self.values_dict['intens1']['ranges'], \
             key=lambda item: item['max'])

        self.values_dict['intens2'] = self['intensity2'].getProperties()
        self.values_dict['intens2']['value'] = None
        self.values_dict['intens2']['ranges'] = []
        for intens_range in self['intensity2']['ranges']:
            temp_intens_range = {}
            temp_intens_range['max'] = intens_range.CurMax
            temp_intens_range['index'] = intens_range.CurIndex
            temp_intens_range['offset'] = intens_range.CurOffset
            self.values_dict['intens2']['ranges'].append(temp_intens_range)
        #Sort the ranges accordingly to the maximal values
        self.values_dict['intens2']['ranges'] = \
             sorted(self.values_dict['intens2']['ranges'], \
             key=lambda item: item['max'])
        ...
        self.chan_intens_mean = self.getChannelObject('intensMean1')
        if self.chan_intens_mean is not None:
            self.chan_intens_mean.connectSignal('update', self.intens_mean_1_changed)

        self.chan_intens_mean = self.getChannelObject('intensMean2')
        if self.chan_intens_mean is not None:
            self.chan_intens_mean.connectSignal('update', self.intens_mean_2_changed)
    ...
    def intens_mean_1_changed(self, value):
        """
    ...
    """
        if state == 'opened':
            ...
            if self.cmd_set_intens_acq_time is not None:
                self.cmd_set_intens_acq_time(self.values_dict['intens1'] \
                     ['acqTimeOnOpenMs']) 
                self.cmd_set_intens_acq_time(self.values_dict['intens2'] \
                     ['acqTimeOnOpenMs']) 
        else:
            self.shutter_is_opened = False
            if self.cmd_set_intens_acq_time is not None:
    ...
    def update_values(self):
        ...
        values_to_send.append(self.values_dict['intens1']['value'])
        values_to_send.append(self.values_dict['intens2']['value'])
    ...
    def get_values(self):
        ...
        val['intens1'] = val['intens1']['value']
        val['intens2'] = val['intens2']['value']
        return val
